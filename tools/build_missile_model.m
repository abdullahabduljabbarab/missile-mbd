...
function build_missile_model()
    ...
    assignin('base', 'V_M_INIT',    V_M_INIT);     % lead-collision-course launch velocity
...
function buildGuidanceSubsystem(model)
    ...
    % TPN_Law: a_cmd = N * Vc * omega. N baked in from missile_params.m.
    N_val = evalin('base', 'N');
    addMatlabFcn(sys, 'TPN_Law', ...
        ...
        '%%#codegen\n' ...
        '    N = %g;              %% baked in at build time from missile_params.m\n' ...
        '    a_cmd = N * Vc * omega;\n' ...
        'end\n'], N_val), ...
        [430 80 560 160]);

    % Sat_a_cmd: direction-preserving magnitude saturation.
    % LATAX_MAX baked in from missile_params.m.
    LATAX_MAX_val = evalin('base', 'LATAX_MAX');
    addMatlabFcn(sys, 'Sat_a_cmd', ...
        {'a_cmd'}, {'a_cmd_sat'}, ...
        sprintf([ ...
        'function a_cmd_sat = fcn(a_cmd)\n' ...
        '%%#codegen\n' ...
        '    LATAX_MAX = %g;   %% baked in at build time from missile_params.m\n' ...
        '    mag = norm(a_cmd);\n' ...
        '    if mag > LATAX_MAX\n' ...
...
function buildKinematicSubsystem(model)
    ...
    % Integrate_v: velocity integrator, init condition = lead-collision-
    % course launch velocity V_M_INIT (computed in missile_params.m from
    % target geometry). Represents a launcher that computed lead before
    % firing, exactly as real interceptor launchers do.
    add_block('built-in/Integrator', [sys '/Integrate_v'], ...
        'Position',       [130 140 190 200], ...
        'InitialCondition','V_M_INIT');
...
function buildTerminationSubsystem(model)
    ...
    % Intercept_Check. R_LETHAL baked in from missile_params.m.
    R_LETHAL_val = evalin('base', 'R_LETHAL');
    addMatlabFcn(sys, 'Intercept_Check', ...
        {'r_T', 'r_M'}, {'hit'}, ...
        sprintf([ ...
        'function hit = fcn(r_T, r_M)\n' ...
        '%%#codegen\n' ...
        '    R_LETHAL = %g;   %% baked in at build time from missile_params.m\n' ...
        '    hit = double(norm(r_T - r_M) < R_LETHAL);\n' ...
        'end\n'], R_LETHAL_val), ...
        [130 60 260 120]);

    % Timeout_Check. T_MAX baked in from missile_params.m.
    T_MAX_val = evalin('base', 'T_MAX');
    addMatlabFcn(sys, 'Timeout_Check', ...
        {'t'}, {'timeout'}, ...
        sprintf([ ...
        'function timeout = fcn(t)\n' ...
        '%%#codegen\n' ...
        '    T_MAX = %g;   %% baked in at build time from missile_params.m\n' ...
        '    timeout = double(t >= T_MAX);\n' ...
        'end\n'], T_MAX_val), ...
        [130 150 260 200]);

    % LOS_Reversal_Check with persistent peak tracking.
    % Persistent state requires discrete sample time (not continuous).
    addMatlabFcn(sys, 'LOS_Reversal_Check', ...
        ...
        '        Vc_peak = Vc;\n' ...
        '    end\n' ...
        '    reversed = double((Vc_peak > 0) && (Vc < 0));\n' ...
        'end\n']), ...
        [130 230 260 280]);
    setDiscreteSampleTime([sys '/LOS_Reversal_Check']);

    % Priority latch: first-triggered wins, then held.
    % Inputs are doubles (0.0 / 1.0); treat non-zero as true.
    % Persistent state requires discrete sample time.
    addMatlabFcn(sys, 'Priority_Latch', ...
        ...
        'end\n']), ...
        [330 130 460 200]);
    setDiscreteSampleTime([sys '/Priority_Latch']);
...
function setDiscreteSampleTime(block_path)
    % MATLAB Function blocks with persistent state need a discrete
    % sample time. Bind to the model's fixed step so the block ticks
    % at every solver step rather than trying to run continuous.
    rt = sfroot;
    chart = rt.find('-isa', 'Stateflow.EMChart', '-and', 'Path', block_path);
    if isempty(chart)
        return
